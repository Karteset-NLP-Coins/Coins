...
# removing dups
antoninus_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Antoninus Pius')
antoninus_list = list(dict.fromkeys(antoninus_df[0].tolist()))

domitian_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Domitian')
domitian_list = list(dict.fromkeys(domitian_df[0].tolist()))

hadrian_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Hadrian')
hadrian_list = list(dict.fromkeys(hadrian_df[0].tolist()))

lucius_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Lucius Verus')
lucius_list = list(dict.fromkeys(lucius_df[0].tolist()))

marcus_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Marcus Aurelius')
marcus_list = list(dict.fromkeys(marcus_df[0].tolist()))

nerva_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Nerva')
nerva_list = list(dict.fromkeys(nerva_df[0].tolist()))

titus_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Titus')
titus_list = list(dict.fromkeys(titus_df[0].tolist()))

trajan_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='Trajan')
trajan_list = list(dict.fromkeys(trajan_df[0].tolist()))

vespasian_df = pd.read_excel(path_start + 'emp url ds numismatics.xlsx', header=None, sheet_name='VESPASIAN')
